...
layui.define(['jquery'],function (exports) {
    ...
    var settings = {
        base_url: modPath
        , images_upload_url: '/admin/file/upload'//图片上传接口
        , language: 'zh_CN'
        , response: {
        ...
        , success: function (res, succFun, failFun) {//上传完成回调
            if (res[this.response.statusName] == this.response.statusCode.ok) {
                succFun(res[this.response.dataName]['location']);
            } else {
    ...
    t.render = function (option) {
        ...
        option.plugins = option.plugins ? option.plugins : ' media axupimgs bdmap quickbars print preview searchreplace autolink fullscreen image link media codesample table charmap hr advlist lists wordcount imagetools indent2em';

        option.toolbar = option.toolbar ? option.toolbar : 'media axupimgs bdmap preview undo redo | forecolor backcolor bold italic underline strikethrough | indent2em alignleft aligncenter alignright alignjustify outdent indent | link bullist numlist image table codesample | formatselect fontselect fontsizeselect';
        ...
        Object.assign(option,{images_upload_base_path:'/upload'});
        console.log(option);
        tinymce.init(option);
